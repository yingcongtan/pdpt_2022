...
def sort_truck_by_workhr_slackness(truck_list, truck_yCycle_file, DT_sol, DT_b_sol, verbose = 0):
    ...
    for t_key, t_value in truck_list.items():
        ...
        aa_, bb_ = DT_sol[(t_dest, t_key)], DT_sol[(t_origin, t_key)]
        print(max([v for v in DT_sol.values()]))
        print(f'DT_sol[(t_dest, t_key): {aa_}')
        print(f'DT_sol[(t_origin, t_key): {bb_}')
        print(f'total_workhr:{total_workhr}')
        print(f't_max_workhr:{t_max_workhr}')
...
def rank_truck_by_slackness_distance(pdpt_ins, ini_sol, verbose = 0):
    sorted_truck_list = {}

    truck_route = ini_sol['route']['truck_route']
    cargo_route = ini_sol['route']['cargo_route']

    truck_yCycle_file = ini_sol['route']['truck_yCycle']
    ...
    for cargo_key in undelivered_cargos:
        ...
        truck_slack, slack_sorted = sort_truck_by_workhr_slackness(truck_list, truck_yCycle_file, 
                                                        D_sol, Db_sol, verbose = 0)

        print(truck_slack, slack_sorted)
        if verbose>0:
        ...
        truck_route_file_ = {t_key: truck_route[t_key] for t_key in slack_sorted}
